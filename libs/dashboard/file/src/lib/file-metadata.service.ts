...
export class FileMetadataService {
  ...
  async getFileSize(path: string): Promise<number> {
    try {
      const result = await this.serial.executeCommand(
        `stat --format=%s -- ${FileUtils.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileModificationTime(path: string): Promise<Date> {
    try {
      const result = await this.serial.executeCommand(
        `stat --format=%Y -- ${FileUtils.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileAttributes(path: string): Promise<FileAttributes> {
    try {
      const result = await this.serial.executeCommand(
        `stat --format="%a %U %G %s %Y %n" -- ${FileUtils.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async showFileDetails(path: string): Promise<string> {
    try {
      const result = await this.serial.executeCommand(
        `ls -la -- ${FileUtils.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileMimeType(path: string): Promise<string> {
    try {
      const result = await this.serial.executeCommand(
        `file --mime-type -b -- ${FileUtils.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileMd5(path: string): Promise<string> {
    try {
      const result = await this.serial.executeCommand(
        `md5sum -- ${FileUtils.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileSha256(path: string): Promise<string> {
    try {
      const result = await this.serial.executeCommand(
        `sha256sum -- ${FileUtils.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getDirectorySize(path: string): Promise<number> {
    try {
      const result = await this.serial.executeCommand(
        `du -sb ${FileUtils.escapePath(path)} | cut -f1`,
        'pi@raspberrypi:',
        10000
      );
