...
export class FileSearchService {
  ...
  async getFileLine(path: string, lineNumber: number): Promise<string> {
    try {
      const result = await this.serial.executeCommand(
        `sed -n '${lineNumber}p' ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileHead(path: string, lineCount = 10): Promise<string[]> {
    try {
      const result = await this.serial.executeCommand(
        `head -n ${lineCount} ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileTail(path: string, lineCount = 10): Promise<string[]> {
    try {
      const result = await this.serial.executeCommand(
        `tail -n ${lineCount} ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async compareFiles(path1: string, path2: string): Promise<string> {
    try {
      const result = await this.serial.executeCommand(
        `diff ${this.escapePath(path1)} ${this.escapePath(
          path2
  ...
  async previewReplace(
    ...
    try {
      const result = await this.serial.executeCommand(
        `sed 's/${searchTerm}/${replaceTerm}/g' ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async getFileRange(
    ...
    try {
      const result = await this.serial.executeCommand(
        `sed -n '${startLine},${endLine}p' ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
