...
export class SerialFacadeService {
   ...
   * データストリーム (Observable)
   */
  readonly data$ = this.reader.data$;
  ...
  async connect(baudRate: number = 115200): Promise<boolean> {
    try {
      ...
      this.writer.initialize(this.currentPort);

      // Reader の起動（バックグラウンドで実行）
      this.reader.startReading(this.currentPort).catch((error) => {
  ...
  async write(data: string): Promise<void> {
    return this.writer.writeSync(data);
  }
  ...
  async readString(): Promise<string> {
    return this.reader.readStringOnce();
  }
