...
export class FileOperationService {
  ...
  async moveFile(
    ...
    try {
      ...
      const command = `${sudoPrefix}mv -- ${this.escapePath(
        fromPath
  ...
  async copyFile(
    ...
    try {
      const sudoPrefix = useSudo ? 'sudo ' : '';
      const command = `${sudoPrefix}cp -- ${this.escapePath(
        fromPath
      )} ${this.escapePath(toPath)}`;
  ...
  async renameFile(
    ...
    try {
      await this.serial.executeCommand(
        `mv -- ${this.escapePath(oldPath)} ${this.escapePath(newPath)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async changeFilePermissions(
    ...
    try {
      await this.serial.executeCommand(
        `chmod ${permissions} -- ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async changeFileOwner(
    ...
    try {
      ...
      await this.serial.executeCommand(
        `chown ${owner}${groupArg} -- ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async createSymbolicLink(
    ...
    try {
      await this.serial.executeCommand(
        `ln -s -- ${this.escapePath(target)} ${this.escapePath(linkPath)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async createHardLink(
    ...
    try {
      await this.serial.executeCommand(
        `ln -- ${this.escapePath(target)} ${this.escapePath(linkPath)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async createDirectory(
    ...
    try {
      ...
      await this.serial.executeCommand(
        `mkdir ${flag}-- ${this.escapePath(path)}`,
        'pi@raspberrypi:',
        10000
      );
  ...
  async removeDirectory(
    ...
    try {
      const command = recursive
        ? `rm -r -- ${this.escapePath(path)}`
        : `rmdir -- ${this.escapePath(path)}`;
